    ...
    def draw(self):
        if not self.is_left_col:
            ...
            for key in self.view.body_parts_ordered:
                self.view.draw_image( self.view.model.get_image( self.animal.list_body_parts[key].active_sec) , Vec(1300,220))
            #adn
        else :
            self.view.draw_image("dna_window", Vec(0,56))
            for key in self.view.body_parts_ordered:
                self.view.draw_image( self.view.model.get_image( self.animal.list_body_parts[key].active_sec) , Vec(74,220))
            #adn
